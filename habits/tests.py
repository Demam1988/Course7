...
class HabitTestCase(APITestCase):
    ...
    def test_habit_create(self):
        ...
        self.assertTrue(Habit.objects.filter(action="test_action_create").exists())
    ...
    def test_habit_bad_create(self):
        ...
        response = self.client.post(reverse("habits:habit_create"), data=data_bad)
        self.habit.refresh_from_db()

        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)

        self.assertEqual(Habit.objects.all().count(), 1)
        self.assertFalse(Habit.objects.filter(action="test_action_bad_create").exists())
    ...
    def test_habit_update(self):
        ...
        response = self.client.patch(
            reverse("habits:habit_update", args=[self.habit.pk]), data=data
        )
        self.habit.refresh_from_db()

        self.assertEqual(response.status_code, status.HTTP_200_OK)

        self.assertEqual(self.habit.place, "test_place_update")

    def test_habit_destroy(self):
        """Тест удаления привычки"""

        self.client.delete(reverse("habits:habit_delete", args=[self.habit.pk]))
