...
class UserTestCase(APITestCase):
    ...
    def test_user_create(self):
        ...
        data = {"email": "user1@test.com", "password": "12345", "chat_id": "12345123"}
    ...
    def test_user_create_without_chat_id(self):
        ...
        data = {"email": "user1@test.com", "password": "12345"}
        ...
        self.assertEqual(
            capturedOutput.getvalue(), "chat_id нужен для " "отправки уведомлений\n"
        )
    ...
    def test_user_update(self):
        ...
        response = self.client.patch(
            reverse("users:user_update", args=[self.user.pk]), data=data
        )
        self.user.refresh_from_db()

        self.assertEqual(self.user.chat_id, "12345123")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
